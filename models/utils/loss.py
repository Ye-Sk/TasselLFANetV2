...
def bbox_iou(box1, box2, CIoU=True, EIoU=False, eps=1e-7):
    # Returns Intersection over Union (IoU) of box1(1,4) to box2(n,4)

    # Get the coordinates of bounding boxes (x1, y1, x2, y2 = box1)
    if EIoU:  # transform from xywh to xyxy
...
class ComputeLoss:
    ...
    def __call__(self, p, targets, img=None, epoch=0):
        ...
        # bbox loss
        if fg_mask.sum():
            loss[0], loss[2], iou = self.reg_loss(pred_distri, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask)
